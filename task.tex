...
\vspace{-1mm} In this paper, we use italic for emphasis, Courier for abbreviations like \texttt{RE}, bold italic for the first appearance
of a concept like \emph{\textbf{clue words}}, Courier surrounded by / for regular expressions like {\small
\texttt{/list(\;the)?\;\_\_AIRLINE/}}, underlined italic for words of sentences in our dataset like \underline{\textit{Boston}}.
...
\REtags are the same as the target slot labels. % because our goal is to annotate clue words for certain slot labels.
...
Intuitively, complicated \REs can lead to better performance but require more efforts to generate. %, \RE complexity is often an
 Generally, there are two aspects affecting \RE complexity most: the number of \RE groups
