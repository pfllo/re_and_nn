...
returns
any texts that match the pattern. %The search is designed to return \red{every match on a sentence}.
%If  a sentence is matched by the given pattern,
%its corresponding \REtag is assigned to either the sentence (for intent detection) or some phrases in the sentence (for slot filling).
...
\textbf{\emph{\RE groups}} (the text surrounded by parenthese). (1)~For the method in Sec.~\ref{interact_with_module}, since we need to
...
number of \RE groups often leads to better, finer-grained\cyan{??} precision but lower coverage on pattern matching, while a larger number of
