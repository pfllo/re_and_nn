...
\label{re_in_exp} We use the standard \RE grammar in this work. Our \REs are written by an \red{paid?unpaid?} annotator 
...
to write the slot \REs for \red{the methods in Sec.~\ref{fusion_with_input} and \ref{fusion_with_output}}, for which it took around
 1 hour to write the 60 \REs with averagely 1.7 groups. By contrast, writing slot \REs to guide attention requires more
efforts as the annotator needs to carefully select clue words and \red{targets???} for the full slot label. As a result, it took about 
