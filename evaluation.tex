...
\label{re_in_exp} Our \texttt{RE}s are written \orange{in Perl syntax} by a person who is familiar with the dataset. We only use the 20-shot data to write
...
minutes to yield one pattern, and 54 patterns are collected in total \orange{(used 1.5 hours)}, with averagely 2.2 (from 1 to 4) \RE groups for each \RE (the group
...
and 60 patterns are produced \orange{(used 70 minutes)}, with averagely 1.7 (from 1 to 3) \RE groups. However, writing slot patterns to guide attention is more
...
requires 2-5 minutes, and 115 patterns \orange{(used 5.5 hours)} with averagely 3.3 (from 2 to 8) \RE groups are produced. The performance of the intent \texttt{RE}s
...
state in total how long did it take to derive all the REs. State which \RE convention is used. } \orange{LUO to ZW: see the newly added orange parts}
