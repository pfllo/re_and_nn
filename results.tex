...
\caption{\cyan{Results on Full Few-Shot Learning. Slot filling does not distinguish full/partial few-shot.}}
\label{tab_full_few}
\end{table*}
...
\cyan{the shorter distance that the gradient needs to travel from the loss to these parameters}
% the shorter distance from these parameters
make \texttt{logit} easier to train. 
However, since \texttt{logit} modifies the output directly, the final prediction is more sensitive to the insufficiently trained \cyan{weights} in \texttt{logit}, which leads to its inferior results in the 5-shot setting.
...
and its  context word \textsl{\underline{from}} may have been % Since this simple context is easily
...
\footnote{For compactness, we only combine the best-performing fusion method in each task with \texttt{mem}, while others can be easily combined as well.}
% \footnote{
...
Further, since \texttt{feat} concatenates \REtags at the input level, the powerful \NN \cyan{following the \REtag} makes it more likely to overfit than \texttt{logit}, therefore \texttt{feat} performs even worse than \BLSTM.
...
\cyan{We can also see that \texttt{two+both} works clearly better than the state-of-art method (\LL) in intent detection, which jointly models the two tasks. And \texttt{mem+feat} is comparative to \LL in slot filling.}
