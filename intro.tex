...
making it widely used in industry and especially in scenarios where the training data is limited \red{(few-shot learning)}~\cite{gc2015big}.
...
often leading to lower coverage. Therefore, in practice, \RE is often combined with \red{data-driven methods like neural network}
...
be used to improve \red{data-driven models like \NN}. For example, a\RE itself often indicates the informative words
(\textbf{\textit{clue words}}) \red{within its pattern}. %for its prediction.

%Since \NN proves to perform generally well in vairous \NLP tasks~\cite{goldberg2017neural}, 
In this paper,
...
we are able to examine both high-precision \REs and those \red{of low quality}, % low-precision \REs, 
