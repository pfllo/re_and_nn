...
such, it is widely used in industry, especially when the number of available training examples is limited \cyan{(few-shot learning)}~\cite{gc2015big}.\lb{few-shot learning need to be defined}. \lb{GC15 supports \RE are used when data is limited}
...
By doing so,  %different from the scenario where we only use high-precision \REs to handle a fraction of cases, 
we are able to utilize both high-precision \REs (but are time-consuming to generate) and those \red{of low quality} (but are easy to write), % low-precision \REs, 
