...
they often have a low coverage of \z{low coverage of what??}. To overcome this limitation, \REs are often used together with data-driven
methods like neural network (\NN) based techniques, where a set of carefully-written \REs are used to handle certain cases with high
precision, leaving the rest for a data-driven method.
...
A \RE often encodes a developer's knowledge of the problem domain, by indicating e.g., the informative words (\textbf{\textit{clue words}})
within its pattern.
...
This paper proposes a novel approach to combine \REs with \NNs at different levels.  At the input layer, we propose to use the evaluation
